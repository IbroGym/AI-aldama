...
export function KioskEmergencyActions({ stopName }: KioskEmergencyActionsProps) {
  ...
  const copy = useMemo(() => {
    ...
    return {
      police: {
        title: t("kiosk.callPolice"),
        description: `When you click "${t("kiosk.callPolice")}", the police will arrive${place} in 3 minutes.`,
        confirm: t("kiosk.callPolice"),
      },
      ambulance: {
        title: t("kiosk.callAmbulance"),
        description: `When you click "${t("kiosk.callAmbulance")}", the ambulance will arrive within 9 minutes${place}.`,
        confirm: t("kiosk.callAmbulance"),
      },
      decline: t("kiosk.decline"),
    }
  }, [stopName, t])
    ...
    <>
      <div className="flex items-center justify-between gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-slate-900">{t("kiosk.emergency")}</div>
          <div className="mt-0.5 text-xs text-slate-500">{t("kiosk.sosOrAmbulance")}</div>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <Button
            ...
            aria-label={t("kiosk.callAmbulance")}
            title="Ambulance"
          >
